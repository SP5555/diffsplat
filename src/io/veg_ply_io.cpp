...
// Reinterpret int16 bits as IEEE 754 float16, convert to float32.
...
static std::vector<PlyElement> parseHeader(std::ifstream &file, const std::string &path)
{
    ...
    std::string line; bool done = false;
    while (std::getline(file, line)) {
        ...
        } else if (line == "end_header") { done = true; break; }
    }
    if (!done) log_fatal("VegPLYLoader", "Malformed PLY (no end_header): " + path);
    return elems;
}
...
static std::vector<VegGaussian3D> readGaussians(std::ifstream   &file,
                                                ...
                                                const Codebook   &cb)
{
    // Build prop name -> byte offset in row
    ...
    const auto &cb_rot_re  = cb.at("rotation_re");  // w component
    const auto &cb_rot_im  = cb.at("rotation_im");  // x, y, z components
    ...
    for (int i = 0; i < elem.count; i++) {
        ...
        // Positions: int16 bits -> float16 -> float32
...
std::vector<VegGaussian3D> VegPLYLoader::load(const std::string &path)
{
    ...
    if (!codebook_elem)
        log_fatal("VegPLYLoader", "No 'codebook_centers' element in: " + path +
                  " (file must be saved with quantised=True)");

    // gaussians comes before codebook_centers in the binary section.
    // Skip gaussians, read codebook, then reopen and read gaussians.
    ...
    // Reopen and re-parse header to land at the binary start again
    file.open(path, std::ios::binary);
    parseHeader(file, path);

    auto splats = readGaussians(file, *gaussians_elem, codebook);
    log_info("VegPLYLoader", "Loaded " + std::to_string(splats.size()) + " VEG gaussians from " + path);
    return splats;
}
